...
export class Radiant6CanadaEncoder {
   ...
   * (liftck_player dev/playbackFiles CanadaPoleDebug.log, GREAT_LAKES.xml:1).
   *
   * The player has no 2010 branch — both the legacy VJ (Radiant6VirtualJournal.java:157-163)
   * and CK Player 2.0 (Radiant6CanadaMessageParser.ts:223) raise CASHIER_RECOGNIZED from
   * any line carrying OperatorId/OperatorName *before* the EventId switch, then let this
   * one fall through unhandled. So 2010 is chosen for wire realism, not because it is routed.
   */
